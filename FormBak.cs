...
namespace Bak
{
	...
	public struct BAK_SUB_FILE
	{
		...
		public long Size;
        public bool IsChangeBackupFile;

		public BAK_SUB_FILE(int nDummy)
		{
			sFileName = "";
            IsChangeBackupFile = false;
			Size = 0;
		}
	}
  ...
  //========================================================
	...
	struct BAK_ITEM_DEF
	{
        ...
        public bool CanBaseOnPriorBackup;
    ...
    private bool m_bDeleteFiles = false;
        ...
        private string m_sBaseOnPriorBackupCMDSwitch;
        private bool m_bBaseOnPriorBackup;
        private string m_sChangedFilesBackupCMDSwitch;
        private bool m_bChangedFilesBackup;
        private DateTime m_dateBakStart;
        private string m_sPriorFullBackupFile;
        private DateTime m_dateBaseBakStart;

        private string m_sBakFileRegExp = "^(.+?)(\\d+)_(\\d+)_(\\d\\d\\d\\d)(c?)\\.";
        ...
        private string m_sBackupFilePrefixWithPath;
        BAK_FILE_DEF m_PriorBackupFiles = null;
		...
		private void SetStatus( string sStatusText )
	  {
		...
		private void ReadJobParams()
		{
            ...
            m_sBaseOnPriorBackupCMDSwitch = "";
            try { m_sBaseOnPriorBackupCMDSwitch = m_xmlnodeJob.GetAttribute("BaseOnPriorBackupCMDSwitch"); }      catch { m_sBaseOnPriorBackupCMDSwitch = ""; }

            if (m_sBaseOnPriorBackupCMDSwitch != "")
                m_bBaseOnPriorBackup = Utils.SwitchIsPresentInCMD(m_sBaseOnPriorBackupCMDSwitch);
            else
                m_bBaseOnPriorBackup = false;

            m_sChangedFilesBackupCMDSwitch = "";
            try { m_sChangedFilesBackupCMDSwitch = m_xmlnodeJob.GetAttribute("ChangedFilesBackupCMDSwitch"); }
            catch { m_sChangedFilesBackupCMDSwitch = ""; }

            if (m_sChangedFilesBackupCMDSwitch != "")
                m_bChangedFilesBackup = Utils.SwitchIsPresentInCMD(m_sChangedFilesBackupCMDSwitch);
            else
                m_bChangedFilesBackup = false;

            if (m_bBaseOnPriorBackup && m_bChangedFilesBackup) //Conflicting switches
                m_bBaseOnPriorBackup = false;

            
            m_dateBaseBakStart = new DateTime(1990, 1, 1); //Reasonable old
            m_dateBakStart = m_dateBaseBakStart;
			...
			for( int nNodeIdx=0; nNodeIdx< nNodeCount; nNodeIdx++ )
			{
                ...
                if (xmlnodCurrItem.GetAttribute("CanBaseOnPriorBackup") == "NO")
                    BakItem.CanBaseOnPriorBackup = false;
                else
                    BakItem.CanBaseOnPriorBackup = true;
    ...
    //------------------------------------------------------------------------------------
		...
		private bool CheckJobParams()
	  {
		...
		//---------------------------------------------------------------------------
        private void SetBakFileNameAndLatestBackupDate()
	  {
            string sChangeBackup = "";
            string sBackupType = "Full backup";
            ...
            m_sBackupFilePrefixWithPath = GetFileNameInDestDir(m_sArcFilePrefix, m_sArcSubdir).ToUpper();
            m_sPriorFullBackupFile = "";

            if( m_bChangedFilesBackup )
            {
                SetLatestFullBackupDateAndFile();
                if (m_dateBakStart > m_dateBaseBakStart)
                {
                    WriteBakLog("\r\n>>>>>>>>>>>>>Prior full backup: " + m_sPriorFullBackupFile + " - we will only backup new or changed files since full backup was done.\r\n");
                    sChangeBackup = "c";
                    sBackupType = "Changes since " + m_sPriorFullBackupFile;
                }
                else
                    WriteBakLog("\r\nPrior full backup not found. Doing full backup!\r\n");

           }

			 //Warning! If you change this also change the regular expression in FreeDiskSpaceByDeletingOldBackups
			m_sBackupFileName = string.Format( "{0}{1}_{2}_{3}{5}.{4}", m_sArcFilePrefix, dateNow.Month, dateNow.Day, dateNow.Year, m_sArcFileExt, sChangeBackup );
			...
			AddSummary("");
			if( m_sArcSubdir != "" )
			  AddSummary( m_sBakDevice +  m_sArcSubdir + " \\ " +  m_sBackupFileName + " (" + sBackupType + ")" );
			else
              AddSummary(m_sBakDevice + m_sBackupFileName + " (" + sBackupType + ")" );
	 }
		...
		private void GetAllFilesInDirectory( string sDirectory, string sFileMask, ref ArrayList vecFiles )
	  {
		...
		private void ReadExistingBakFiles()
		{
			...
			foreach( string sFile in vecFiles )
			{
				...
				if(  ! GetBackupFileNameItems(sFile, out match ) )
					continue;

				//Date of file is encoded in the file name
				int nMon = Convert.ToInt32( match.Groups[2].Value );
				int nDay = Convert.ToInt32( match.Groups[3].Value );
				int nYear = Convert.ToInt32( match.Groups[4].Value );
                string sChangeBakFile = match.Groups[5].Value;
                ...
                SubFile.IsChangeBackupFile = sChangeBakFile.Length > 0;
	...
	private	void SortBakFilesListByDate( ref ArrayList  vec_Files )
	{
        ...
        //-------------------------------------------------------------------------------
        private bool GetBackupFileNameItems(string sFileName, out Match match)
        {
            Regex RExp = new Regex(m_sBakFileRegExp + m_sArcFileExt + "$", RegexOptions.IgnoreCase);
            match = RExp.Match( sFileName );
            return match.Success;
        }

        //-------------------------------------------------------------------------------
        private int GetAncestorBackupsCount(ref BAK_FILE_DEF FileDef, string sFileNameWithPrefix)
        {
            
            int i;
            BAK_FILE_DEF CurrFileDef;
            BAK_SUB_FILE SubFile;
            string sFileName;
            Match match;
            int nResult;
            ...
            sFileNameWithPrefix = sFileNameWithPrefix.ToUpper();


            //Locate in m_vecBackup
            for (i = m_vecBakFiles.Count-1; i >= 0; i--)
            {
                CurrFileDef = (BAK_FILE_DEF) m_vecBakFiles[i];
                if( CurrFileDef.dateOfFile == FileDef.dateOfFile )
                    break;
            }
            ...
            {
                CurrFileDef = (BAK_FILE_DEF) m_vecBakFiles[i];
                foreach (object SubFileObj in CurrFileDef.vecSubFiles)
                {
                    SubFile = (BAK_SUB_FILE)SubFileObj;
                    sFileName = SubFile.sFileName.ToUpper();
                    if (sFileName.IndexOf(sFileNameWithPrefix) >= 0) //Found such more recent backup!
                        if (GetBackupFileNameItems(sFileName, out match))
                        {
                            if (match.Groups[5].Value != "") //Change backup file
                            {
                                nResult++;
                                break;
                            }
                            else
                                return nResult; //Stop at full backup file
                        }
        ...
        //-------------------------------------------------------------------------------
        private bool AreThereFullBackupsWithAncestors(ref BAK_FILE_DEF FileDef)
        {
            string sFileNameWithPrefix;
            Match match;
            ...
            foreach (object SubFileObj in FileDef.vecSubFiles)
            {
                ...
                {
                   string sChangeBakFile = match.Groups[5].Value;
                   sFileNameWithPrefix = match.Groups[1].Value;
                   if (sChangeBakFile.Length == 0) //This is full backup file - let's find its ancestors
                   {
                       if (GetAncestorBackupsCount(ref FileDef, sFileNameWithPrefix) > 0)
                           return true;
		...
		private void DeleteBackupFiles()
		{
			...
			{
				BakFile = (BAK_FILE_DEF) BakFileObj;
			...
			while( nN < 5 * 365 )
			{
                ...
                {
                    bool bHaveFileToKeep = false;
                    for (int i = 0; i < vecFilesInInterval.Count; i++)
                    {
                        BAK_FILE_DEF FileInInterval = (BAK_FILE_DEF)vecFilesInInterval[i];
                        if (i < vecFilesInInterval.Count - 1 || bHaveFileToKeep )
                        {
                            if (AreThereFullBackupsWithAncestors(ref FileInInterval))
                            {
                                sInfo += "KEEP(FULL BACKUP): ";
                                bHaveFileToKeep = true;
                            }
                            else
                            {
			...
			{
                BakFile = (BAK_FILE_DEF) m_vecBakFiles[m_vecBakFiles.Count-1];

				sInfo +=  "\r\nNo files to delete by the interval algorithm!\r\n" +
					"Deleting the oldest backup: " + BakFile.GetFileNames() + "\r\n";
				vecFilesToDelete.Add( BakFile );
			...
			{
				BakFile = BakFileObj as BAK_FILE_DEF;
...
//			SetBakFileName();
            ...
            m_PriorBackupFiles = null;
            BAK_FILE_DEF FileDef = null;
            for (int i = 0; i< m_vecBakFiles.Count; i++)
            {
                FileDef = (BAK_FILE_DEF)m_vecBakFiles[i];
                //Check existance
   ...
   //------------------------------------------------------------------------------------
		...
		private string ProcessStringParams( string sValue, string sLocation )
		{
            ...
            sValue = sValue.Replace("{starty}", m_dateBakStart.Year.ToString() );
            sValue = sValue.Replace("{startm}", m_dateBakStart.Month.ToString() );
            sValue = sValue.Replace("{startd}", m_dateBakStart.Day.ToString() );
		...
		private void ProcessAfterCmd( BAK_ITEM_DEF BakItem)
		{
			...
			ExecuteShellCommand( BakItem.sAfterCMDEXE,  sAfterCMDPARAMS, BakItem.bAfterCMDWriteStdErrToLog, wShowWindow, ! BakItem.bAfterCmdNowait, BakItem.nMaxWaitTime_Sec );
    ...
    //------------------------------------------------------------------------------------
        private void CopyPriorBackup()
        {
            bool bCopied = false;


            if (m_PriorBackupFiles == null)
                return;

            if( m_PriorBackupFiles.vecSubFiles.Count == 0 )
                return;

            WriteBakLog("Copying prior back file to base new backup on...\r\n");
            //Find backup file with the current prefix
            foreach (object SubFileObj in m_PriorBackupFiles.vecSubFiles)
            {
                BAK_SUB_FILE SubFile = (BAK_SUB_FILE)SubFileObj;
                
                string sFileNameNormal = SubFile.sFileName;
                string sFileNameUpper = sFileNameNormal.ToUpper();


                if (sFileNameUpper.IndexOf(m_sBackupFilePrefixWithPath) >= 0)
                {
                    FileInfo finf = new FileInfo( sFileNameNormal );
                    bCopied = true;
                    WriteBakLog("Copying from '" + sFileNameNormal + "' to '" + m_sBackupFileNameWithPath + "'...");

                    try
                    {
                        finf.CopyTo(m_sBackupFileNameWithPath, true);
                        WriteBakLog( "Done.\r\n");
                    }
                    catch (System.Exception ex)
                    {
                        WriteBakLog("*Error: " + ex.Message + "\r\n");
                   }
                    break;
                }
            }
            ...
            if( !bCopied )
                WriteBakLog( "Corresponding file to base backup on not found\r\n");
        }

        private void SetLatestFullBackupDateAndFile()
        {
            BAK_FILE_DEF FileDef;
            FileInfo finf;
            string sFileNameUpper;
            ...
            for(int i = 0; i < m_vecBakFiles.Count; i++)
            {
			  ...
			  {
				BAK_SUB_FILE SubFile = (BAK_SUB_FILE) SubFileObj;
                if( SubFile.IsChangeBackupFile )
                    continue;

				sFileNameUpper =  SubFile.sFileName.ToUpper();

                if (sFileNameUpper.IndexOf(m_sBackupFilePrefixWithPath) == 0)
                {
                    finf = new FileInfo(SubFile.sFileName);
                    if (finf.Exists)
                    {
                        m_sPriorFullBackupFile = SubFile.sFileName;
                        m_dateBakStart = FileDef.dateOfFile;
                        return;
                    }
                }
    ...
    //------------------------------------------------------------------------------------
		private void RunJob()
		{
			...
			foreach( BAK_ITEM_DEF CurrBakItem in m_vecItems  )
			{
                ...
                //Always copy prior backup for first item unless disabled
                ...
                if (bNewArcFile)
                {
                    SetBakFileNameAndLatestBackupDate();
                }

                bCopyPrior = m_bBaseOnPriorBackup && bNewArcFile && CurrBakItem.CanBaseOnPriorBackup && !m_bChangedFilesBackup;
...
//                    if (!m_bBaseOnPriorBackup)
//                      File.Delete(m_sBackupFileNameWithPath);
                    ...
                    if (m_bChangedFilesBackup)
                    {
                        FileInfo finf = new FileInfo( m_sBackupFileNameWithPath);
                        if( !finf.Exists ) //Probably no files to backup - let's create an empty zip file
                        {
                            WriteBakLog( "*Warning: Backup file  was not created. Probably there are no files changed from the prior full backup '" + m_sPriorFullBackupFile + "'.\r\n" );
                            WriteBakLog( "Saving an empty file instead of zip file.\r\n" );


					      try
					      {
						    StreamWriter stream = new StreamWriter(m_sBackupFileNameWithPath, true ); //, System.Text.Encoding.GetEncoding("utf-8")  );
//						    stream.WriteLine( "Seems no files for backup. Please see the prior full backup file:\r\n" + m_sPriorFullBackupFile + "\r\n" );
						    stream.Close();
					      }
					      catch( Exception  E )
					      {
                              WriteBakLog( "*Error creating file '" + m_sBackupFileNameWithPath + "': " + E.Message + "\r\n" );
  ...
  //====================================================================================
	...
	{
        public ArrayList vecBaseFiles = null; //Set of base files (full backup, not partial)
		public ArrayList vecSubFiles = null; //Set of file of one date backup (partial or full backup)
